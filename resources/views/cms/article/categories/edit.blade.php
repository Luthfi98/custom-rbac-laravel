                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('categories-article.update', $category->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <select name="parent" id="parent" class="form-control select2">
                                ...
                                @foreach ($parents as $parentMenu)
                                    <option {{ $parentMenu->id == $category->parent_id }} value="{{ $parentMenu->id }}">{{ $parentMenu->name }}</option>
                                    @foreach ($parentMenu->child as $childMenu)
                                        <option {{ $childMenu->id == $category->parent_id }} value="{{ $childMenu->id }}">- {{ $childMenu->name }}</option>
                                        @foreach ($childMenu->child as $subChildMenu)
                                            <option {{ $subChildMenu->id == $category->parent_id }} value="{{ $subChildMenu->id }}">-- {{ $subChildMenu->name }}</option>
                                        @endforeach
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <br>
                            <img src="{{ $category->image ? url($category->image) : '' }}" alt="No Image" id="preview">
